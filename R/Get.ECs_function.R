...

 {
  ...
  ndays.ECs<-cbind(Ndays.per.stage,Ndays_Sow2Flw,Ndays_Flw2Egf)
    ...
    Sum.rain.per.stage <- matrix(NA,
                                   ...
                                   dimnames = list(Envs, c(paste("TotRain_", interval.names, sep = ""),
  ...
  mean.temp.per.stage <- matrix(NA,nrow = length(Envs), ncol = length(interval.names),
                                dimnames = list(Envs, paste("Avtemp_", interval.names, sep = "")))
  mean.max.temp.per.stage <- matrix(NA,nrow = length(Envs), ncol = length(interval.names),
                                    dimnames = list(Envs, paste("Avmaxtemp_", interval.names, sep = "")))
  mean.min.temp.per.stage <- matrix(NA,nrow = length(Envs), ncol = length(interval.names),
                                    dimnames = list(Envs, paste("AvMintemp_", interval.names, sep = "")))
  MinTbelow0per.stage <- matrix(NA,nrow = length(Envs), ncol = length(interval.names)+1,
                                dimnames = list(Envs, c(paste("Ndays<0_", interval.names, sep = ""),"Ndays<0_Flw")))
  MaxToverr26per.stage <- matrix(NA,nrow = length(Envs), ncol = length(interval.names),
                                 dimnames = list(Envs, paste("Ndays>26_", interval.names, sep = "")))
  MaxToverr34per.stage <- matrix(NA,nrow = length(Envs), ncol = length(interval.names),
  ...
  for (e in 1:length(Envs)) {
    ...
    MinTbelow0per.stage[e, paste("Ndays<0_", interval.names, sep = "")] <- sapply(2:ncol(all.env.stages), 
  ...
  temp.ECS<-cbind(mean.temp.per.stage,mean.max.temp.per.stage,mean.min.temp.per.stage,
  ...
  AveSR.per.stage <- matrix(NA,nrow = length(Envs), ncol = length(interval.names),
                            dimnames = list(Envs, paste("AveSR_", interval.names, sep = "")))
  AvePQ.per.stage <- matrix(NA,nrow = length(Envs), ncol = length(interval.names),
  ...
  AveVPD.per.stage <- matrix(NA,nrow = length(Envs), ncol = length(interval.names),
