...
As well as weather related ECs, soil attribute covariates can be extracted using the `get.S.ECs()` function from the [SLGA](https://esoil.io/TERNLandscapes/Public/Pages/SLGA/index.html) data resource [@grundy2015].
...
The tiff files of soil maps from the SLGA can be downloaded to a local directory with the `dl.slga()` function. This directory of files can then be specified with the `tif.dir` argument of the `get.S.ECs()` rather downloading large files repeatedly.

For more information on how ECs are calculated, see @fradgley2025.

## Get ECs for a wider set of unobserved environments

Make a set of location coordinates within the Australian cropping belt with the `crop.locs()` function and repeat the locations for a few years.
```{r}
wheat.area.envs <- crop.locs(res=0.2)
yrs <- 2018:2020
wheat.area.envs <- cbind(Year=rep(yrs,each=nrow(wheat.area.envs)), wheat.area.envs)
wheat.area.envs$Env <- paste(wheat.area.envs$Year, wheat.area.envs$Loc, sep="_")
```
...
wheat.area.wthr <- get.SILO.weather(
  ...
  verbose = T, ncores = 1, plus.yr = T
)

wheat.area.wthr <- add.SMI(wheat.area.wthr)

wheat.area.envs$sow.dates <- paste0("15/05/",wheat.area.envs$Year)
